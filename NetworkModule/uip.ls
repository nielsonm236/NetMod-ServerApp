 ...
 158                     ; 238     ++uip_acc32[1];
 160  002b 725c0028      	inc	_uip_acc32+1
 161                     ; 239     if (uip_acc32[1] == 0) {
 163  002f 2604          	jrne	L56
 164                     ; 240       ++uip_acc32[0];
 166  0031 725c0027      	inc	_uip_acc32
 167  0035               L56:
 168                     ; 244   if (uip_acc32[3] < (op16 & 0xff)) {
 170  0035 c6002a        	ld	a,_uip_acc32+3
 171  0038 5f            	clrw	x
 172  0039 97            	ld	xl,a
 ...
 180                     ; 245     ++uip_acc32[2];
 182  0044 725c0029      	inc	_uip_acc32+2
 183                     ; 246     if (uip_acc32[2] == 0) {
 185  0048 260a          	jrne	L17
 186                     ; 247       ++uip_acc32[1];
 188  004a 725c0028      	inc	_uip_acc32+1
 189                     ; 248       if (uip_acc32[1] == 0) {
 191  004e 2604          	jrne	L17
 192                     ; 249         ++uip_acc32[0];
 194  0050 725c0027      	inc	_uip_acc32
 195  0054               L17:
 196                     ; 253 }
 ...
 480  0004 89            	pushw	x
 481  0005 ae00e3        	ldw	x,#_uip_buf+14
 482  0008 89            	pushw	x
 483  0009 5f            	clrw	x
 ...
 561  0003 c600e5        	ld	a,_uip_buf+16
 562  0006 5f            	clrw	x
 563  0007 97            	ld	xl,a
 564  0008 4f            	clr	a
 565  0009 cb00e6        	add	a,_uip_buf+17
 ...
 584  0021 89            	pushw	x
 585  0022 ae00ef        	ldw	x,#_uip_buf+26
 586  0025 89            	pushw	x
 ...
 596  0031 89            	pushw	x
 597  0032 ae00f7        	ldw	x,#_uip_buf+34
 598  0035 89            	pushw	x
 ...
 714  001e 42            	mul	x,a
 715  001f 724f0044      	clr	(_uip_conns+25,x)
...
1052  000e 1c002b        	addw	x,#_uip_conns
...
1522  0003 ce00cf        	ldw	x,_uip_conn
1523  0006 1f04          	ldw	(OFST-1,sp),x
1525                     ; 486   uip_sappdata = uip_appdata = &uip_buf[UIP_IPTCPH_LEN + UIP_LLH_LEN];
1527  0008 ae010b        	ldw	x,#_uip_buf+54
1528  000b cf00d3        	ldw	_uip_appdata,x
1529  000e cf0014        	ldw	_uip_sappdata,x
...
1572                     ; 514     uip_len = 0;
1574  0048 5f            	clrw	x
1575  0049 cf00d1        	ldw	_uip_len,x
1576                     ; 515     uip_slen = 0;
1578  004c cf0012        	ldw	_uip_slen,x
...
1640  0090 35800026      	mov	_uip_flags,#128
...
1647  0097 35140104      	mov	_uip_buf+47,#20
...
1703  00dc c70104        	ld	_uip_buf+47,a
...
1710  00e2 35040026      	mov	_uip_flags,#4
...
1731  00f4 35080026      	mov	_uip_flags,#8
...
1826  0163 c600ec        	ld	a,_uip_buf+23
...
1878  0191               L7001:
1879  0191 cf00f9        	ldw	_uip_buf+36,x
1880                     ; 681   uip_ipaddr_copy(BUF->destipaddr, BUF->srcipaddr);
1882  0194 ce00ef        	ldw	x,_uip_buf+26
1883  0197 cf00f3        	ldw	_uip_buf+30,x
1886  019a ce00f1        	ldw	x,_uip_buf+28
1887  019d cf00f5        	ldw	_uip_buf+32,x
1888                     ; 682   uip_ipaddr_copy(BUF->srcipaddr, uip_hostaddr);
1890  01a0 ce0022        	ldw	x,_uip_hostaddr
1891  01a3 cf00ef        	ldw	_uip_buf+26,x
1894  01a6 ce0024        	ldw	x,_uip_hostaddr+2
1895  01a9 cf00f1        	ldw	_uip_buf+28,x
1896                     ; 685   goto send;
...
1905  01af ae002b        	ldw	x,#_uip_conns
...
1936  01e3 cf00cf        	ldw	_uip_conn,x
1937                     ; 907   uip_flags = 0;
1939  01e6 725f0026      	clr	_uip_flags
1940                     ; 911   if (BUF->flags & TCP_RST) {
1942  01ea 7204010403cc  	btjf	_uip_buf+47,#2,L1411
1943                     ; 912     uip_connr->tcpstateflags = UIP_CLOSED;
1945  01f2 6f19          	clr	(25,x)
1946                     ; 913     uip_flags = UIP_ABORT;
1948  01f4 35200026      	mov	_uip_flags,#32
...
2009  0238 d60044        	ld	a,(_uip_conns+25,x)
2010  023b 2703cc02e3    	jrne	L3701
2011                     ; 799       uip_connr = &uip_conns[c];
2013  0240 1c002b        	addw	x,#_uip_conns
...
2026  024f a104          	cp	a,#4
2027  0251 25cb          	jrult	L5301
2029  0253 cc0685        	jra	L575
2030  0256               L5401:
2031                     ; 740   BUF->flags = TCP_RST | TCP_ACK;
2034  0256 35140104      	mov	_uip_buf+47,#20
2035                     ; 741   uip_len = UIP_IPTCPH_LEN;
2037  025a ae0028        	ldw	x,#40
2038  025d cf00d1        	ldw	_uip_len,x
2039                     ; 742   BUF->tcpoffset = 5 << 4;
2041  0260 35500103      	mov	_uip_buf+46,#80
2042                     ; 745   c = BUF->seqno[3];
2044  0264 5500fe0003    	mov	L73_c,_uip_buf+41
2045                     ; 746   BUF->seqno[3] = BUF->ackno[3];
2047  0269 55010200fe    	mov	_uip_buf+41,_uip_buf+45
2048                     ; 747   BUF->ackno[3] = c;
2050  026e 5500030102    	mov	_uip_buf+45,L73_c
2051                     ; 749   c = BUF->seqno[2];
2053  0273 5500fd0003    	mov	L73_c,_uip_buf+40
2054                     ; 750   BUF->seqno[2] = BUF->ackno[2];
2056  0278 55010100fd    	mov	_uip_buf+40,_uip_buf+44
2057                     ; 751   BUF->ackno[2] = c;
2059  027d 5500030101    	mov	_uip_buf+44,L73_c
2060                     ; 753   c = BUF->seqno[1];
2062  0282 5500fc0003    	mov	L73_c,_uip_buf+39
2063                     ; 754   BUF->seqno[1] = BUF->ackno[1];
2065  0287 55010000fc    	mov	_uip_buf+39,_uip_buf+43
2066                     ; 755   BUF->ackno[1] = c;
2068  028c 5500030100    	mov	_uip_buf+43,L73_c
2069                     ; 757   c = BUF->seqno[0];
2071  0291 5500fb0003    	mov	L73_c,_uip_buf+38
2072                     ; 758   BUF->seqno[0] = BUF->ackno[0];
2074  0296 5500ff00fb    	mov	_uip_buf+38,_uip_buf+42
2075                     ; 759   BUF->ackno[0] = c;
2077  029b 55000300ff    	mov	_uip_buf+42,L73_c
2078                     ; 764   if (++BUF->ackno[3] == 0) {
2080  02a0 725c0102      	inc	_uip_buf+45
2081  02a4 2610          	jrne	L7401
2082                     ; 765     if (++BUF->ackno[2] == 0) {
2084  02a6 725c0101      	inc	_uip_buf+44
2085  02aa 260a          	jrne	L7401
2086                     ; 766       if (++BUF->ackno[1] == 0) {
2088  02ac 725c0100      	inc	_uip_buf+43
2089  02b0 2604          	jrne	L7401
2090                     ; 767         ++BUF->ackno[0];
2092  02b2 725c00ff      	inc	_uip_buf+42
2093  02b6               L7401:
2094                     ; 773   tmp16 = BUF->srcport;
2096  02b6 ce00f7        	ldw	x,_uip_buf+34
2097  02b9 cf0000        	ldw	L34_tmp16,x
2098                     ; 774   BUF->srcport = BUF->destport;
2100  02bc ce00f9        	ldw	x,_uip_buf+36
2101  02bf cf00f7        	ldw	_uip_buf+34,x
2102                     ; 775   BUF->destport = tmp16;
2104  02c2 ce0000        	ldw	x,L34_tmp16
2105  02c5 cf00f9        	ldw	_uip_buf+36,x
2106                     ; 778   uip_ipaddr_copy(BUF->destipaddr, BUF->srcipaddr);
2108  02c8 ce00ef        	ldw	x,_uip_buf+26
2109  02cb cf00f3        	ldw	_uip_buf+30,x
2112  02ce ce00f1        	ldw	x,_uip_buf+28
2113  02d1 cf00f5        	ldw	_uip_buf+32,x
2114                     ; 779   uip_ipaddr_copy(BUF->srcipaddr, uip_hostaddr);
2116  02d4 ce0022        	ldw	x,_uip_hostaddr
2117  02d7 cf00ef        	ldw	_uip_buf+26,x
2120  02da ce0024        	ldw	x,_uip_hostaddr+2
2121  02dd cf00f1        	ldw	_uip_buf+28,x
...
2139  02f2 d60045        	ld	a,(_uip_conns+26,x)
...
2150  0302 1c002b        	addw	x,#_uip_conns
...
2172  031c cf00cf        	ldw	_uip_conn,x
...
2251  037b c60103        	ld	a,_uip_buf+46
...
2265  038f d6010b        	ld	a,(_uip_buf+54,x)
...
2290  03aa d6010c        	ld	a,(_uip_buf+55,x)
...
2298  03b6 d6010e        	ld	a,(_uip_buf+57,x)
2299  03b9 5f            	clrw	x
...
2306  03c2 d6010d        	ld	a,(_uip_buf+56,x)
2307  03c5 5f            	clrw	x
...
2402  0446 c600d1        	ld	a,_uip_len
2403  0449 97            	ld	xl,a
2404  044a c600d2        	ld	a,_uip_len+1
...
2411  0457 cf00d1        	ldw	_uip_len,x
...
2420  0464 c60104        	ld	a,_uip_buf+47
...
2468  04b5 c600ff        	ld	a,_uip_buf+42
2469  04b8 c10027        	cp	a,_uip_acc32
2470  04bb 85            	popw	x
...
2587  0543 35010026      	mov	_uip_flags,#1
...
2624  0582 72010026f8    	btjf	_uip_flags,#0,L356
...
2660  05a3 7200002603cc  	btjf	_uip_flags,#0,L1021
2662  05ab c60104        	ld	a,_uip_buf+47
2663  05ae a43f          	and	a,#63
2664  05b0 a112          	cp	a,#18
2665  05b2 26f4          	jrne	L1021
2666                     ; 1024         if((BUF->tcpoffset & 0xf0) > 0x50) {
2668  05b4 c60103        	ld	a,_uip_buf+46
...
2682  05c8 d6010b        	ld	a,(_uip_buf+54,x)
...
2707  05e3 d6010c        	ld	a,(_uip_buf+55,x)
...
2799  0665 35420026      	mov	_uip_flags,#66
...
2805                     ; 1070         uip_len = 0;
2807  066f 5f            	clrw	x
2808  0670 cf00d1        	ldw	_uip_len,x
2809                     ; 1071         uip_slen = 0;
2811  0673               LC002:
2812  0673 cf0012        	ldw	_uip_slen,x
...
2834  0685 7204010403cc  	btjf	_uip_buf+47,#2,L5401
...
2841  0690 7201010430    	btjf	_uip_buf+47,#0,L3321
...
2890  06c5 720b01041f    	btjf	_uip_buf+47,#5,L1421
2891                     ; 1123         uip_appdata = ((char *)uip_appdata) + ((BUF->urgp[0] << 8) | BUF->urgp[1]);
2893  06ca c60109        	ld	a,_uip_buf+52
2894  06cd 97            	ld	xl,a
2895  06ce c6010a        	ld	a,_uip_buf+53
2896  06d1 02            	rlwa	x,a
2897  06d2 72bb00d3      	addw	x,_uip_appdata
2898  06d6 cf00d3        	ldw	_uip_appdata,x
2899                     ; 1124         uip_len -= (BUF->urgp[0] << 8) | BUF->urgp[1];
2901  06d9 c60109        	ld	a,_uip_buf+52
2902  06dc 97            	ld	xl,a
2903  06dd c6010a        	ld	a,_uip_buf+53
2904  06e0 02            	rlwa	x,a
2905  06e1 72b000d1      	subw	x,_uip_len
2906  06e5 50            	negw	x
2907  06e6 cf00d1        	ldw	_uip_len,x
2908  06e9               L1421:
2909                     ; 1132       if (uip_len > 0 && !(uip_connr->tcpstateflags & UIP_STOPPED)) {
2911  06e9 ce00d1        	ldw	x,_uip_len
...
2963  0732 c60026        	ld	a,_uip_flags
...
2973  073f 720900261e    	btjf	_uip_flags,#4,L5521
2974                     ; 1188           uip_slen = 0;
2976  0744 5f            	clrw	x
2977  0745 cf0012        	ldw	_uip_slen,x
...
3055  079d ce0014        	ldw	x,_uip_sappdata
3056  07a0 cf00d3        	ldw	_uip_appdata,x
3057                     ; 1234 	if (uip_slen > 0 && uip_connr->len > 0) {
3059  07a3 ce0012        	ldw	x,_uip_slen
...
3165  0832 725f0106      	clr	_uip_buf+49
3166  0836 725f0105      	clr	_uip_buf+48
...
3187  084e ce00d1        	ldw	x,_uip_len
...
3196  0856 7201010419    	btjf	_uip_buf+47,#0,L1031
3197                     ; 1274         if (uip_flags & UIP_ACKDATA) {
3199  085b 1e04          	ldw	x,(OFST-1,sp)
3200  085d 720100260c    	btjf	_uip_flags,#0,L3031
...
3230  0874 720100260d    	btjf	_uip_flags,#0,L7031
...
3246  0886 ce00d1        	ldw	x,_uip_len
...
3254  088d ce00d1        	ldw	x,_uip_len
...
3263  0895 7201010417    	btjf	_uip_buf+47,#0,L7131
...
3302  08b8 7201002666    	btjf	_uip_flags,#0,L356
...
3316  08c5 35100104      	mov	_uip_buf+47,#16
3317  08c9 cc075a        	jra	L736
3318  08cc               L5331:
3319                     ; 1381     BUF->wnd[0] = ((UIP_RECEIVE_WINDOW) >> 8);
3321  08cc 35010105      	mov	_uip_buf+48,#1
3322                     ; 1382     BUF->wnd[1] = ((UIP_RECEIVE_WINDOW) & 0xff);
3324  08d0 35b80106      	mov	_uip_buf+49,#184
3325  08d4               L546:
3326                     ; 1389   tcp_send_noconn:
3326                     ; 1390   BUF->ttl = UIP_TTL;
3328  08d4 354000eb      	mov	_uip_buf+22,#64
3329                     ; 1391   BUF->len[0] = (uint8_t)(uip_len >> 8);
3331  08d8 5500d100e5    	mov	_uip_buf+16,_uip_len
3332                     ; 1392   BUF->len[1] = (uint8_t)(uip_len & 0xff);
3334  08dd 5500d200e6    	mov	_uip_buf+17,_uip_len+1
3335                     ; 1394   BUF->urgp[0] = BUF->urgp[1] = 0;
3337  08e2 725f010a      	clr	_uip_buf+53
3338  08e6 725f0109      	clr	_uip_buf+52
3339                     ; 1397   BUF->tcpchksum = 0;
3341  08ea 5f            	clrw	x
3342  08eb cf0107        	ldw	_uip_buf+50,x
3343                     ; 1398   BUF->tcpchksum = ~(uip_tcpchksum());
3345  08ee cd0000        	call	_uip_tcpchksum
3347  08f1 53            	cplw	x
3348  08f2 cf0107        	ldw	_uip_buf+50,x
3349                     ; 1403   ip_send_nolen:
3349                     ; 1404 
3349                     ; 1405   BUF->vhl = 0x45;
3351  08f5 354500e3      	mov	_uip_buf+14,#69
3352                     ; 1406   BUF->tos = 0;
3354  08f9 725f00e4      	clr	_uip_buf+15
3355                     ; 1407   BUF->ipoffset[0] = BUF->ipoffset[1] = 0;
3357  08fd 725f00ea      	clr	_uip_buf+21
3358  0901 725f00e9      	clr	_uip_buf+20
3359                     ; 1408   ++ipid;
3361  0905 ce0008        	ldw	x,L11_ipid
3362  0908 5c            	incw	x
3363  0909 cf0008        	ldw	L11_ipid,x
3364                     ; 1409   BUF->ipid[0] = (uint8_t)(ipid >> 8);
3366  090c 55000800e7    	mov	_uip_buf+18,L11_ipid
3367                     ; 1410   BUF->ipid[1] = (uint8_t)(ipid & 0xff);
3369  0911 55000900e8    	mov	_uip_buf+19,L11_ipid+1
3370                     ; 1412   BUF->ipchksum = 0;
3372  0916 5f            	clrw	x
3373  0917 cf00ed        	ldw	_uip_buf+24,x
3374                     ; 1413   BUF->ipchksum = ~(uip_ipchksum());
3376  091a cd0000        	call	_uip_ipchksum
3378  091d 53            	cplw	x
3379  091e cf00ed        	ldw	_uip_buf+24,x
3381  0921 2004          	jra	L222
3382  0923               L356:
3383                     ; 1430   drop:
3383                     ; 1431   uip_len = 0;
3385  0923 5f            	clrw	x
3386  0924 cf00d1        	ldw	_uip_len,x
3387                     ; 1432   uip_flags = 0;
3389                     ; 1433   return;
3390  0927               L222:
3392  0927 725f0026      	clr	_uip_flags
...
3747  000a 000000000000  	ds.b	8
3748                     	xdef	_uip_listenports
3749  0012               _uip_slen:
3750  0012 0000          	ds.b	2
3751                     	xdef	_uip_slen
3752  0014               _uip_sappdata:
3753  0014 0000          	ds.b	2
...
3760  0016               _uip_mqttserveraddr:
3761  0016 00000000      	ds.b	4
3762                     	xdef	_uip_mqttserveraddr
3763  001a               _uip_draddr:
3764  001a 00000000      	ds.b	4
3765                     	xdef	_uip_draddr
3766  001e               _uip_netmask:
3767  001e 00000000      	ds.b	4
3768                     	xdef	_uip_netmask
3769  0022               _uip_hostaddr:
3770  0022 00000000      	ds.b	4
3771                     	xdef	_uip_hostaddr
3772                     	xdef	_uip_process
3773  0026               _uip_flags:
3774  0026 00            	ds.b	1
3775                     	xdef	_uip_flags
3776  0027               _uip_acc32:
3777  0027 00000000      	ds.b	4
3778                     	xdef	_uip_acc32
3779  002b               _uip_conns:
3780  002b 000000000000  	ds.b	164
3781                     	xdef	_uip_conns
3782  00cf               _uip_conn:
3783  00cf 0000          	ds.b	2
3784                     	xdef	_uip_conn
3785  00d1               _uip_len:
3786  00d1 0000          	ds.b	2
3787                     	xdef	_uip_len
3788  00d3               _uip_appdata:
3789  00d3 0000          	ds.b	2
...
3796  00d5               _uip_buf:
3797  00d5 000000000000  	ds.b	502
3798                     	xdef	_uip_buf
